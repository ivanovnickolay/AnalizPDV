...
-- Дата скрипта: 27.09.2016 1:03:28
...
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizPDVOutDiffDateOneBranchInnerJoinERPN_tempTable(IN m INT, IN y YEAR, IN b VARCHAR(255))
BEGIN
CREATE TEMPORARY TABLE IF NOT EXISTS temp_diffDateFromBranch  (
  ...
  SELECT num_invoice, date_create_invoice, date_reg_invoice,diffDate, 
...
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizPDVOutDiffDateOneBranchInnerJoinReestr_tempTable(IN m int, IN y year, IN b varchar(255))
...
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizPDVOutDiffDateOneBranchLeftJoinERPN_tempTable(IN m INT, IN y YEAR, IN b VARCHAR(255))
  COMMENT 'Проверка факта включения опаздавших в реестр филиала '
BEGIN
CREATE TEMPORARY TABLE IF NOT EXISTS temp_diffDateFromBranch  (
   key_field varchar(50)DEFAULT NULL ,
  ...
  SELECT num_invoice, date_create_invoice, date_reg_invoice,DATEDIFF(date_reg_invoice, date_create_invoice), 
  type_invoice_full, inn_client, name_client, suma_invoice, pdvinvoice, baza_invoice, 
  name_vendor,num_branch_vendor, num_main_branch FROM erpn_out
      WHERE key_field IN(
