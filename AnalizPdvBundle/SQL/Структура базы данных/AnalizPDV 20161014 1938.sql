...
-- Дата скрипта: 14.10.2016 19:38:27
...
CREATE TABLE erpn_out (
  ...
  PRIMARY KEY (id),
  INDEX IDX_erpn_out (date_create_invoice, inn_client),
  INDEX inn (inn_client),
  INDEX `key` (key_field),
  UNIQUE INDEX Key_fields (num_invoice, date_create_invoice, type_invoice_full, inn_client),
...
CREATE TABLE errorloadreestr (
  ...
  key_field VARCHAR(50) DEFAULT NULL COMMENT 'ключевое поле ',
...
FROM temp_erpn_out_inn_group_numbranch_allUZ
 ...
 WHERE temp_erpn_out_inn_group_numbranch_allUZ.month_create = m
  ...
  AND temp_erpn_out_inn_group_numbranch_allUZ.num_main_branch IS NULL
 ;
END
$$
...
CREATE TEMPORARY TABLE IF NOT EXISTS temp_reestr_out_inn_group_numbranch_allUZ (
  ...
  SELECT
    ...
    temp_reestr_out_inn_group_numbranch_allUZ.num_main_branch AS Reestr_num_main_branch,
    temp_reestr_out_inn_group_numbranch_allUZ.inn_client AS Reestr_inn,
    COALESCE(temp_erpn_out_inn_group_numbranch_allUZ.pdv, 0) AS Erpn_pdv,
    COALESCE(temp_reestr_out_inn_group_numbranch_allUZ.pdv, 0) AS Reestr_pdv,
    COALESCE(temp_erpn_out_inn_group_numbranch_allUZ.pdv, 0) - COALESCE(temp_reestr_out_inn_group_numbranch_allUZ.pdv, 0) AS saldo_pdv
  FROM temp_reestr_out_inn_group_numbranch_allUZ
    LEFT JOIN temp_erpn_out_inn_group_numbranch_allUZ
      ...
      AND temp_reestr_out_inn_group_numbranch_allUZ.num_main_branch = temp_erpn_out_inn_group_numbranch_allUZ.num_main_branch
...
--
-- Описание для процедуры getDocErpnBy_AnalizInnOutInnerJoinBranchAllUZ
--
DROP PROCEDURE IF EXISTS getDocErpnBy_AnalizInnOutInnerJoinBranchAllUZ$$
CREATE DEFINER = 'root'@'%'
PROCEDURE getDocErpnBy_AnalizInnOutInnerJoinBranchAllUZ(IN m INT, IN y INT)
...
BEGIN
  ...
  SELECT
    ...
    ei.name_vendor
  FROM erpn_out ei
  WHERE MONTH(ei.date_create_invoice) = m
...
BEGIN
  CREATE TEMPORARY TABLE IF NOT EXISTS temp_erpn_out_inn_group_numbranch_all (
    month_create int(11) DEFAULT NULL,
    year_create year(4) DEFAULT NULL,
    inn_client varchar(255) DEFAULT NULL,
    pdv decimal(15, 2) DEFAULT NULL,
  ...
  CREATE TEMPORARY TABLE IF NOT EXISTS temp_reestr_out_inn_group_numbranch_all (
    month_create int(11) DEFAULT NULL,
    year_create year(4) DEFAULT NULL,
    inn_client varchar(255) DEFAULT NULL,
    pdv decimal(15, 2) DEFAULT NULL,
...
CREATE TEMPORARY TABLE IF NOT EXISTS Out_InnerJoinAll(
  ...
  (SELECT
    temp_erpn_out_inn_group_numbranch_all.month_create,
    temp_erpn_out_inn_group_numbranch_all.year_create,
    temp_erpn_out_inn_group_numbranch_all.num_main_branch,
    temp_erpn_out_inn_group_numbranch_all.inn_client AS Erpn_Inn,
    temp_reestr_out_inn_group_numbranch_all.inn_client AS Reestr_inn,
    temp_erpn_out_inn_group_numbranch_all.pdv AS Erpn_pdv,
    temp_reestr_out_inn_group_numbranch_all.pdv AS Reestr_pdv,
    temp_erpn_out_inn_group_numbranch_all.pdv - temp_reestr_out_inn_group_numbranch_all.pdv AS saldo_pdv
  FROM temp_erpn_out_inn_group_numbranch_all 
    INNER JOIN temp_reestr_out_inn_group_numbranch_all
      ON temp_erpn_out_inn_group_numbranch_all.month_create = temp_reestr_out_inn_group_numbranch_all.month_create
      AND temp_erpn_out_inn_group_numbranch_all.year_create = temp_reestr_out_inn_group_numbranch_all.year_create
      AND temp_erpn_out_inn_group_numbranch_all.num_main_branch = temp_reestr_out_inn_group_numbranch_all.num_main_branch
      AND temp_erpn_out_inn_group_numbranch_all.inn_client = temp_reestr_out_inn_group_numbranch_all.inn_client
  WHERE temp_erpn_out_inn_group_numbranch_all.month_create = m
  AND temp_erpn_out_inn_group_numbranch_all.year_create = y
  AND (temp_erpn_out_inn_group_numbranch_all.pdv - temp_reestr_out_inn_group_numbranch_all.pdv)<>0);
END
$$
