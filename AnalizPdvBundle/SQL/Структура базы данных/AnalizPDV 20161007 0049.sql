...
-- Дата скрипта: 07.10.2016 0:49:55
...
--
-- Описание для процедуры AnalizInnInInnerJoin
--
DROP PROCEDURE IF EXISTS AnalizInnInInnerJoin$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizInnInInnerJoin(IN month INT, IN year INT)
...
--
-- Описание для процедуры AnalizInnInLeftJoin
--
DROP PROCEDURE IF EXISTS AnalizInnInLeftJoin$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizInnInLeftJoin(IN month INT, IN year INT)
...
--
-- Описание для процедуры AnalizInnInRightJoin
--
DROP PROCEDURE IF EXISTS AnalizInnInRightJoin$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizInnInRightJoin(IN month INT, IN year INT)
...
--
-- Описание для процедуры AnalizInnOutInnerJoinOneBranch_tempTable
--
DROP PROCEDURE IF EXISTS AnalizInnOutInnerJoinOneBranch_tempTable$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizInnOutInnerJoinOneBranch_tempTable(IN m int, IN y int, IN b varchar(255))
BEGIN
...
--
-- Описание для процедуры AnalizInnOutLeftJoinOneBranch_tempTable
--
DROP PROCEDURE IF EXISTS AnalizInnOutLeftJoinOneBranch_tempTable$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizInnOutLeftJoinOneBranch_tempTable(IN m INT, IN y INT, IN b VARCHAR(255))
  COMMENT 'Левое соединение данных ЕРПН и реестра за период по одному филиалу. Вывод только данных из ЕРПН которые не совпали с реестром '
BEGIN
CREATE TEMPORARY TABLE IF NOT EXISTS  temp_erpn_out_inn_group_numbranch (
    month_create int(11)DEFAULT NULL ,
...
--
-- Описание для процедуры AnalizInnOutRightJoinOneBranch_tempTable
--
DROP PROCEDURE IF EXISTS AnalizInnOutRightJoinOneBranch_tempTable$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizInnOutRightJoinOneBranch_tempTable(IN m INT, IN y INT, IN b VARCHAR(255))
  COMMENT 'Правое соединение данных ЕРПН и реестра за период по одному филиалу. Вывод только данных из Реестра которые не совпали с ЕРПН'
BEGIN
CREATE TEMPORARY TABLE IF NOT EXISTS  temp_erpn_out_inn_group_numbranch (
    month_create int(11)DEFAULT NULL ,
...
--
-- Описание для процедуры getDocErpnBy_AnalizInnInInnerJoin
--
DROP PROCEDURE IF EXISTS getDocErpnBy_AnalizInnInInnerJoin$$
CREATE DEFINER = 'root'@'%'
PROCEDURE getDocErpnBy_AnalizInnInInnerJoin(IN month INT, IN year INT)
...
SELECT ei.num_invoice, ei.date_create_invoice, ei.type_invoice_full, 
  ei.inn_client, ei.name_client, ei.pdvinvoice,
...
--
-- Описание для процедуры getDocReestrBy_AnalizInnInInnerJoin
--
DROP PROCEDURE IF EXISTS getDocReestrBy_AnalizInnInInnerJoin$$
CREATE DEFINER = 'root'@'%'
PROCEDURE getDocReestrBy_AnalizInnInInnerJoin(IN month INT, IN year INT)
  COMMENT 'Получает список полученных НН из ЕРПН  по ИПНам которые признаны ошибочними при AnalizInnInInnerJoin'
BEGIN
CALL getTmpTbl_InnInnerJoin (month,year);
SELECT rbi.month, rbi.year, rbi.num_branch, rbi.date_create_invoice, 
