...
-- Дата скрипта: 03.11.2016 19:02:41
...
CREATE TABLE erpn_out_svod_inn (
  ...
  key_field VARCHAR(50) DEFAULT NULL,
  PRIMARY KEY (id),
  INDEX UK_Erpn_out_svod_inn_key_field (key_field)
)
ENGINE = INNODB
AUTO_INCREMENT = 921977
AVG_ROW_LENGTH = 141
CHARACTER SET utf8
COLLATE utf8_general_ci
...
--
-- Описание для процедуры AnalizPDVOutDiffDateAllUZInnerJoinERPN
--
DROP PROCEDURE IF EXISTS AnalizPDVOutDiffDateAllUZInnerJoinERPN$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizPDVOutDiffDateAllUZInnerJoinERPN(IN m INT, IN y INT)
BEGIN
...
--
-- Описание для процедуры AnalizPDVOutDiffDateAllUZInnerJoinReestr
--
DROP PROCEDURE IF EXISTS AnalizPDVOutDiffDateAllUZInnerJoinReestr$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizPDVOutDiffDateAllUZInnerJoinReestr(IN m INT, IN y INT)
  COMMENT 'Проверка факта включения опаздавших выданных НН в реестр филиалов'
BEGIN
  CREATE TEMPORARY TABLE IF NOT EXISTS temp_diffDateFromAll (
    key_field varchar(50) DEFAULT NULL,
    diffDate bigint DEFAULT NULL,
 ...
 AND eo.month_create_invoice=m
  ...
  SELECT
    ...
    rbo.year,
    rbo.num_branch,
    DATE_FORMAT(rbo.date_create_invoice, '%d.%m.%Y'),
    rbo.num_invoice,
    rbo.inn_client,
    rbo.name_client
...
--
-- Описание для процедуры AnalizPDVOutDiffDateAllUZLeftJoinERPN
--
DROP PROCEDURE IF EXISTS AnalizPDVOutDiffDateAllUZLeftJoinERPN$$
CREATE DEFINER = 'root'@'%'
PROCEDURE AnalizPDVOutDiffDateAllUZLeftJoinERPN(IN m INT, IN y INT)
  COMMENT 'Проверка факта включения опаздавших в реестр филиала. Вывод тех которые не включены в реестр  '
BEGIN
CREATE TEMPORARY TABLE IF NOT EXISTS temp_diffDateFromAll  (
   key_field varchar(50)DEFAULT NULL ,
    diffDate bigint DEFAULT NULL ,
  ...
  SELECT num_invoice, DATE_FORMAT(date_create_invoice, '%d.%m.%Y'), DATE_FORMAT(date_reg_invoice, '%d.%m.%Y'),(DATEDIFF(date_reg_invoice, date_create_invoice)-15) AS diff,
  type_invoice_full, inn_client, name_client, suma_invoice, pdvinvoice, baza_invoice,
  name_vendor,num_branch_vendor, num_main_branch FROM erpn_out
      WHERE key_field IN(
...
CALL getTmpTbl_OutLeftJoin(m,y,b);
SELECT
  temp_erpn_out_inn_group_numbranch.month_create,
  temp_erpn_out_inn_group_numbranch.year_create,
  temp_erpn_out_inn_group_numbranch.num_main_branch,
...
CREATE DEFINER = 'root'@'%'
PROCEDURE getTmpTbl_OutBranch(IN m INT, IN y INT, IN b VARCHAR(255))
...
CREATE TEMPORARY TABLE IF NOT EXISTS temp_reestr_out_inn_group_numbranch (
  ...
  WHERE MONTH(`reestrbranch_out`.`date_create_invoice`) = m
  AND YEAR(`reestrbranch_out`.`date_create_invoice`) = y
    AND `reestrbranch_out`.num_branch = b
  GROUP BY MONTH(`reestrbranch_out`.`date_create_invoice`),
           YEAR(`reestrbranch_out`.`date_create_invoice`),
           `reestrbranch_out`.`inn_client`,
           `reestrbranch_out`.`num_branch`);
